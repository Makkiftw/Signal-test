...
class GameWindow < Gosu::Window
	...
	def initialize
		...
		$link_off_color = 0x88ffffff	## White
		$link_on_color = 0x88ff00ff		## Purple
		$led_off_color = 0xff556677 	## Gray
		$led_on_color = 0xffffff00  	## Yellow
		
		## Sprites
		@circle_img = Gosu::Image.new(self, "media/dot_img.png", true)
		@circle_purple_img = Gosu::Image.new(self, "media/dot_purple_img.png", true)
		@dotted_img = Gosu::Image.new(self, "media/dotted_img.png", true)
		...
		for i in 0..ary.length-1
			case ary[i][0]
				...
				when "link"
					self.create_link_id(ary[i][1].to_i, ary[i][2].to_i, ary[i][3])
				when "lever"
		...
		a = $window_width-500
		b = $window_height-200
		@text_field1 = TextField.new(self, @bigfont, a+30, b+64, 6)
		@text_field1.text = "#{@update_delay_max}"
		@text_field2 = TextField.new(self, @bigfont, a+30, b+114, 6)
	...
	def update
		...
		if button_down? Gosu::KbQ
			$camera_zoom = [$camera_zoom * 0.98, 0.25].max
		end
		if button_down? Gosu::KbE
			$camera_zoom = [$camera_zoom / 0.98, 4.0].min
		end
					...
					if @cursor == "node"
						...
						if dists.empty? == false
							...
							if @cursor_input == false
								...
								if @cursor_input != repeater_output
									
									already_linked = false
									
									$links.each_with_index   { |inst, i|  
									if inst.input == @cursor_input and inst.output == repeater_output
									...
									if already_linked == false
										self.create_link(@cursor_input, repeater_output, @cursor)
										# puts "Link created from #{@cursor_input.id} to #{repeater_output.id}"
									end
								end
						...
						if @copy_data != false
							...
							t_total = Gosu::milliseconds - t_start
							puts "time1: #{t_total} ms"
							t_start = Gosu::milliseconds
							...
							for i in 0..data_links.length-1
								...
								input = new_nodes[data_links[i][0]]
								output = new_nodes[data_links[i][1]]
								
								self.create_link(input, output, type)
								
							end
							
							t_total = Gosu::milliseconds - t_start
							puts "time5: #{t_total} ms"
	...
	def draw
		...
		if @display_grid == true
			...
			col = 0x1affffff
			for i in 0..vert_lines-1		## Draw the vertical lines
				x = (x_edge*1.0/@grid_size).ceil*@grid_size + i*@grid_size - @grid_size/2
				y1 = 0
				y2 = $window_height
				self.draw_line(x*$camera_zoom + $cam_corelate_x, y1, col, x*$camera_zoom + $cam_corelate_x, y2, col, 0)
			end
			for i in 0..hori_lines-1		## Draw the horizontal lines
				y = (y_edge*1.0/@grid_size).ceil*@grid_size + i*@grid_size - @grid_size/2
				x1 = 0
				x2 = $window_width
				self.draw_line(x1, y*$camera_zoom + $cam_corelate_y, col, x2, y*$camera_zoom + $cam_corelate_y, col, 0)
			end
		end
		...
		for i in 0..$nodes.length-1
			xx = $nodes[i].x*$camera_zoom + $cam_corelate_x
			yy = $nodes[i].y*$camera_zoom + $cam_corelate_y
			if self.point_in_rectangle(xx, yy, 0, 0, $window_width, $window_height)
				if $nodes[i].value == 0
					nodes_off_ary << [xx, yy]
				else
					nodes_on_ary << [xx, yy]
		...
		@circle_img.draw_as_points(nodes_off_ary, 2, scale: $camera_zoom)
		@circle_purple_img.draw_as_points(nodes_on_ary, 2, scale: $camera_zoom)
		if @cursor == "repeater" or @cursor == "inverter" or @cursor == "lever" or @cursor == "led"
		...
		if @cursor == "options"
			...
			self.draw_quad(x1, y1, color1, x2, y1, color1, x2, y2, color1, x1, y2, color1, 5)
			self.draw_quad(x1+3, y1+3, color2, x2-3, y1+3, color2, x2-3, y2-3, color2, x1+3, y2-3, color2, 5)
			@font.draw("Optimise Drawing", x1+9, y1+6, 5, 1.0, 1.0, 0xff000000)
			...
			self.draw_quad(x1, y1, color1, x2, y1, color1, x2, y2, color1, x1, y2, color1, 5)
			self.draw_quad(x1+3, y1+3, color2, x2-3, y1+3, color2, x2-3, y2-3, color2, x1+3, y2-3, color2, 5)
			@font.draw("Display Grid", x1+9, y1+6, 5, 1.0, 1.0, 0xff000000)
			
			### Placement Grid toggle
			x1 = a+220
			y1 = b+95
			x2 = a+365
			y2 = b+123
			...
			self.draw_quad(x1, y1, color1, x2, y1, color1, x2, y2, color1, x1, y2, color1, 5)
			self.draw_quad(x1+3, y1+3, color2, x2-3, y1+3, color2, x2-3, y2-3, color2, x1+3, y2-3, color2, 5)
			@font.draw("Placement Grid", x1+9, y1+6, 5, 1.0, 1.0, 0xff000000)
			...
			self.draw_quad(x1, y1, color1, x2, y1, color1, x2, y2, color1, x1, y2, color1, 5)
			self.draw_quad(x1+3, y1+3, color2, x2-3, y1+3, color2, x2-3, y2-3, color2, x1+3, y2-3, color2, 5)
			@font.draw("Draw Links", x1+9, y1+6, 5, 1.0, 1.0, 0xff000000)
	...
	def create_node_id(x, y, value, id)
		...
		@maxid = [id, @maxid+1].max
		
	end
	
	def create_node(x, y, value)
		@maxid += 1
		inst = Node.new(self, x, y, value, @maxid)
		$nodes << inst
		@node_ids << @maxid
		return inst
	end
	...
	def create_link_id(input_id, output_id, type)
		input = self.find_node(input_id)
		output = self.find_node(output_id)
		inst = Link.new(self, input, output, type)
		$links << inst
	end
